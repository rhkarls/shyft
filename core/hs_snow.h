        ...
        namespace hs_snow {
            ...
            struct parameter {
                ...
                parameter(const vector<double>& s, const vector<double>& intervals,
                  ...
                  : s(s), intervals(intervals), tx(tx), cx(cx), ts(ts), lw(lw), cfr(cfr) {
				}
            ...
            struct calculator {
                ...
                template <class R> void step(S& s, R& r, utctime t0, utctime t1, double prec, double temp) const {
                    ...
                    if (snow > 0.0) {
						...
						for (size_t i = 0; i < I.size(); ++i) {
							if (i < idx)
								...
								auto x = sca - I[i];
								auto bin_sp = x * s.sp[i] / 2;
								if (i < I.size() - 1 && p.s[i + 1] > 0) {
									s.sp[i] = bin_sp + snow * p.s[i];
									s.sw[i] = x * s.sw[i] / 2;
								}
								else if (i == I.size() - 1 || (i < I.size() - 1 && p.s[i + 1] == 0)) {
									auto upper = 1.0;
									if (i < I.size() - 1)
										upper = I[i + 1];
									s.sp[i] = 2 * (bin_sp + snow * p.s[i]) / (upper - I[i]);
									s.sw[i] = x * s.sw[i] / (upper - I[i]);
								}
							}
						}
                    ...
                    if (sca < 1.0e-6) swe = 0.0;
                        ...
                        swe = integrate(s.sp, I, I.size(), 0, sca, true);
                        swe += integrate(s.sw, I, I.size(), 0, sca, true);
                    }
...
x_serialize_export_key(shyft::core::hs_snow::state);
