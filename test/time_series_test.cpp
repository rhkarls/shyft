...
class ts_source {
    ...
    size_t n;
public:
	ts_source(utctime start = no_utctime, utctimespan dt = utctimespan{ 0 }, size_t n = 0) : start(start), dt(dt), n(n) {}
    ...
    utcperiod operator()(size_t i) const {
        n_period_calls++;
        if (i > n)
            throw runtime_error("index out of range called");
...
TEST_SUITE("time_series") {
    ...
    TEST_CASE("test_point_timeaxis") {
		...
		TS_ASSERT_EQUALS(ts0.index_of(utctime{ seconds(12) }), std::string::npos);
		vector<utctime> t2 = { utctime{deltahours(1)} }; //just one point
        try {
		...
		utctime t0{ seconds(0) }; utctimespan dt=deltahours(1);
        vector<utctime> t= {t0+1*dt,t0+2*dt,t0+3*dt};
        ...
        TS_ASSERT_EQUALS(tx.index_of(t0-dt),std::string::npos);//(1),utcperiod(t[1],t[2]);
        TS_ASSERT_EQUALS(tx.index_of(t[0]),0u);
        TS_ASSERT_EQUALS(tx.index_of(t[1]-seconds(1)),0u);
        TS_ASSERT_EQUALS(tx.index_of(t[2]+ seconds(1)),std::string::npos);
        TS_ASSERT_EQUALS(tx.open_range_index_of(t[2]+ seconds(1)),1u);
    ...
    TEST_CASE("test_timeaxis") {
        ...
        TS_ASSERT_EQUALS(tx.index_of(t0- seconds(1)),string::npos);
    ...
    TEST_CASE("test_point_source_with_timeaxis") {
        ...
        auto d=deltahours(1);
        size_t n=10;
        vector<utctime> time_points;
        for(size_t i=0; i<=n; i++)
            time_points.emplace_back(t+i*d);
        vector<double> values;
        for(size_t i=0; i<n; ++i)
            values.emplace_back(i*1.0);
    ...
    TEST_CASE("test_hint_based_bsearch") {
        ...
        TS_ASSERT_EQUALS(ta.n_index_of_calls,1u);
        ta.reset_call_count();
    ...
    TEST_CASE("test_average_value_staircase") {
        ...
        vector<point> points_10;
        for(utctime t=t0; t<10*dt+t0; t+=dt)
            points_10.push_back(point(t,to_seconds(t-t0)/to_seconds(dt)) );
    ...
    TEST_CASE("test_average_value_linear_between_points") {
        auto t0 = calendar().time(2000, 1, 1, 0, 0, 0);
        auto dt = deltahours(1);
        size_t n = 3;
        time_axis::fixed_dt tx(t0, dt, n);
        vector<point> points = { point(t0, 1.0), point(t0 + dt / 2, 2.0), point(t0 + 2 * dt, 3) };
        ...
        // case 1: just check it can compute true average..
        size_t ix = 0;
        auto avg1_2_3 = average_value(ps, utcperiod(t0, t0 + 3 * dt), ix,true);
        CHECK(avg1_2_3 == doctest::Approx(2.25));

        // case 2: check that it can deliver true average at a slice of a stair-case
        ix = -1;
        ps.index_of_count = 0;
        TS_ASSERT_DELTA(1.0+ 2*10.5/60, average_value(ps, utcperiod(t0 + deltaminutes(10), t0 + deltaminutes(11)), ix,true), 0.0000001);
        TS_ASSERT_EQUALS(1u, ps.index_of_count);
        TS_ASSERT_EQUALS(ix, 1u);
        // case 3: check that it stop average value after last value observed
        ps.index_of_count = 0;
        ix = 2;
        FAST_CHECK_UNARY(!std::isfinite(average_value(ps, utcperiod(t0 + 5 * dt, t0 + 60 * dt), ix,true)));
        TS_ASSERT_EQUALS(0u, ps.index_of_count);
        TS_ASSERT_EQUALS(ix, 2u);
        // case 4: ask for data before first point -> nan
        ix = 2;
        ps.index_of_count = 0;
        double v = average_value(ps, utcperiod(t0 - 5 * dt, t0 - 4 * dt), ix,true);
        TS_ASSERT(!std::isfinite(v));
        TS_ASSERT_EQUALS(ps.index_of_count, 0u);
        TS_ASSERT_EQUALS(ix, 0u);
        ...
        //outdated TS_ASSERT_DELTA((0.5 * 2 + 3.0) / 1.5, average_value(ps1, full_period, ix,true), 0.00001);
        //outdated TS_ASSERT_DELTA(2.3750, average_value(ps2, full_period, ix,true), 0.00001);
        //outdated TS_ASSERT_DELTA(1.75, average_value(ps3, full_period, ix,true), 0.00001);
        //outdated TS_ASSERT_DELTA(1, average_value(ps4, full_period, ix,true), 0.00001);
        ix = -1;
        v = average_value(ps5, full_period, ix,true);// no points should give nan
        TS_ASSERT(!std::isfinite(v));
        ix = -1;
        v = average_value(ps4, utcperiod(t0 - dt * 10, t0 - dt * 9), ix, true);//before any point should give nan
        TS_ASSERT(!std::isfinite(v));

        ix = 7;// ensure negative direction search ends up in doing a binary -search
        v = average_value(ps6, full_period, ix,true);
        TS_ASSERT_EQUALS(ps6.index_of_count, 1u);
        TS_ASSERT_DELTA(1.5, v, 0.00001);
        v = average_value(ps6, utcperiod(t0 + 3 * dt, t0 + 4 * dt), ix,true);
        TS_ASSERT_EQUALS(ps6.index_of_count, 1u);
        TS_ASSERT_DELTA(3.5, v, 0.00001);
        TS_ASSERT_EQUALS(ix, 4u);
        ix = 0;
        ps6.index_of_count = 0;
        v = average_value(ps6, utcperiod(t0 + 7 * dt, t0 + 8 * dt), ix, true);
        TS_ASSERT_EQUALS(ps6.index_of_count, 1u);
        ...
        //outdated TS_ASSERT_DELTA(avg_a.value(2), (3 * 1.0) / 1.0, 0.000001);//(1*0.5+2*1.5+3*1.0)/3.0
        TS_ASSERT_EQUALS(ps.index_of_count, 0u);
    }
    ...
    TEST_CASE("accessor_policy") {
        ...
        SUBCASE("accumulate") {
            ...
            CHECK(a_default.value(ta.size()-1)==doctest::Approx(to_seconds(deltahours(1))*(1.0+2.0+2.5+2.5)));
            CHECK(a_0.value(ta.size()-1)==doctest::Approx(to_seconds(deltahours(1))*(1.0+2.0+2.5)));
            CHECK(!std::isfinite(a_nan.value(ta.size()-1)));
        }
    }
    ...
    TEST_CASE("test_point_timeseries_with_point_timeaxis") {
        ...
        vector<utctime> times= {t0 + deltahours(1),t0 + deltahours(2),t0 + deltahours(3),t0 + deltahours(4)};
    ...
    TEST_CASE("test_time_series_difference") {
		...
		const utctime T0{ seconds(0) };
		const utctime T1{ seconds(100) };
        for (size_t i = 0; i < n; ++i)
    ...
    TEST_CASE("test_binary_operator") {
        ...
        */
        ...
        using namespace shyft;
        calendar utc;
        utctime start = utc.time(2016,3,8);
        utctimespan dt = deltahours(1);
        size_t  n = 4;
        double a_value=3.0;
        double b_value=2.0;
...

        {
            ...
            typedef point_ts<ta_t> ts_t;
			vector<utcperiod> sub_periods({ utcperiod(utctime{deltaminutes(10)},utctime{deltaminutes(20)}) });
        ...
        }
        {
            ...
            for(size_t i=0; i<n; ++i)
                periods.emplace_back(start+i*dt,start+(i+1)*dt);
            ta_t ta(periods);
            auto  a = make_shared<ts_t>(ta,a_value);
            ts_t b(ta,b_value); // test by-value as well as shared_ptr<TS>
            test_bin_op<ts_t>(a,b,ta,a_value,b_value);
	...
	TEST_CASE("test_api_ts_stair_nan_after_end") {
		using namespace shyft::api;
		utctime t0{ seconds(0) };
		apoint_ts ts{ gta_t{t0, seconds(1), 3}, 1.0, shyft::time_series::POINT_AVERAGE_VALUE };
		auto a = ts.average(gta_t{ t0,seconds(1),4 });
		auto av = a.values();
	...
	TEST_CASE("test_api_ts_integral_fine_resolution") {
		...
		for (size_t i = 0; i < a.size(); ++i) {
			...
			FAST_CHECK_EQ(av[i], doctest::Approx(to_seconds(deltahours(3))));
		}
	}
	...
	TEST_CASE("extend_calendar_and_fixed_dt") {
		...
		auto utc = make_shared<calendar>();
		utctime t0;
		apoint_ts a{ gta_t{ t0, seconds(1), 3 }, 1.0, shyft::time_series::POINT_AVERAGE_VALUE };
		apoint_ts b{ gta_t{ utc,t0, seconds(1), 10 }, 1.0, shyft::time_series::POINT_AVERAGE_VALUE };
		auto c = a.extend(b, extend_ts_split_policy::EPS_LHS_LAST, extend_ts_fill_policy::EPF_LAST, utctime{}, 0.0);
    ...
    TEST_CASE("test_ts_statistics_speed") {
        ...
        if(verbose)
            cout<<"1 x flatten expression took:"<<msec_cpy<<"ms\n";
        ...
        for(size_t pi=0; pi<3; ++pi) {
            ...
            for (int n_threads = 1; n_threads< 11 ; n_threads += 2) {
                ...
                if(verbose)
                    cout<<"A.statistics speed tests#"<<pi<<"[ "<<n_threads<<"] "<< tad.size() <<" steps, pr.thread = "<< sz << " steps: "<< msec1 << " ms" <<endl;
            }
            vector<tts_t> r2;
            ...
            for (int n_threads = 1; n_threads< 11 ; n_threads += 2) {
                ...
                if(verbose)
                    cout<<"C.statistics speed tests #"<<pi<<"[ "<<n_threads<<"] "<< tad.size() <<" steps, pr.thread = "<< sz << " steps: "<< msec1 << " ms" <<endl;
            }
        }
    ...
    TEST_CASE("test_periodic_ts_concept") {
        ...
        struct periodic_ts {
            ...
            int map_index(utctime tt) const {
				double t = to_seconds(tt.time_since_epoch());
                t -= to_seconds(t0.time_since_epoch());
                if (t < 0 || to_seconds(period) < t) {
                    t = fmod(t, to_seconds(period));
                    if (t < 0)
                        t += to_seconds(period);
                }
                return round(double(t) / double(to_seconds(dt)));
            }
        ...
        TS_ASSERT_DELTA(fun(t0 + dt/2 - seconds(1)), profile[0], 1e-9);
        TS_ASSERT_DELTA(fun(t0 + dt/2), profile[1], 1e-9);
        TS_ASSERT_DELTA(fun(t0 - dt/2 - seconds(1)), profile[7], 1e-9);
    ...
    TEST_CASE("test_periodic_template_ts") {
        ...
        for (size_t i = 0; i < expected_fl.size(); ++i)
            CHECK(fl.value(i)==doctest::Approx(expected_fl[i]));
    }
    ...
    TEST_CASE("test_accumulate_value") {
        ...
        vector<double> values;
        for (size_t i = 0; i < n; ++i)
            values.emplace_back(i!= 5?i*1.0:shyft::nan);//0, 1,2,4,nan,6..9
        //Two equal time_axis::fixed_dt representations
        ...
        TS_ASSERT_DELTA(0.5*to_seconds(deltahours(1)), y1, 0.0001);
        TS_ASSERT_EQUALS(tsum, deltahours(1));
        auto y2 = accumulate_value(a, utcperiod(ta.time(0), ta.time(2)), last_ix, tsum, true);
        TS_ASSERT_DELTA(1.0*to_seconds(deltahours(2)), y2, 0.0001);
        TS_ASSERT_EQUALS(tsum, deltahours(2));
        auto y3 = accumulate_value(a, utcperiod(ta.time(0), ta.time(6)), last_ix, tsum, true,false);// last part of accumulation is nan, so 4 flattens out
        TS_ASSERT_DELTA(2.0*to_seconds(deltahours(4))+4.0*to_seconds(deltahours(1)), y3, 0.0001);
    ...
    TEST_CASE("test_accumulate_ts_and_accessor") {
        ...
        // simple test at ix=1:
        TS_ASSERT_DELTA(0.5*to_seconds(deltahours(1)), aa.value(1), 0.001);
        TS_ASSERT_DELTA(0.5*to_seconds(deltahours(1)), ats.value(1), 0.001);
        // now the ts have some extra feature through it's f(t), operator()
        TS_ASSERT_DELTA(0.25*to_seconds(deltaminutes(30)), ats(t + deltaminutes(30)), 0.0001);
        // the accessor should be smart, trying to re-use prior computation, I verify the result here,
        TS_ASSERT_DELTA(1.0*to_seconds(deltahours(2)), aa.value(2), 0.0001);// and using step-debug to verify it's really doing the right thing
    }
    TEST_CASE("core_average_ts") {
		...
		vector<double> actual; for (size_t i = 0; i < ats.size(); ++i)actual.push_back(ats.value(i));
        for(size_t i=0; i<ats.size(); ++i) {
			double a = actual[i];
            if(!std::isfinite(expected[i])) {
                FAST_CHECK_EQ(std::isfinite(a),false);
            } else {
                TS_ASSERT_DELTA(a, expected[i], 0.001);
            }
        }
    }
