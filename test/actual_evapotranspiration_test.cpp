...
TEST_CASE("test_water") {
    ...
    double act_evap;
	act_evap = calculate_step(0.0, pot_evap, scale_factor, sca);
    TS_ASSERT_DELTA(act_evap, 0.0, shyfttest::EPS);

	act_evap = calculate_step(1.0e8, pot_evap, scale_factor, sca);
    TS_ASSERT_DELTA(act_evap, pot_evap, shyfttest::EPS);

    TS_ASSERT_DELTA((1 - exp(-3 * 1.0 / 1.0)), calculate_step(1.0, 1.0, 1.0, 0.0), shyfttest::EPS);
}
...
TEST_CASE("test_snow") {
    ...
    const double pot_evap = 5.0; // [mm/h]
    const double scale_factor = 1.0;
	double act_evap_no_snow = calculate_step(water, pot_evap, scale_factor, 0.0);
	double act_evap_some_snow = calculate_step(water, pot_evap, scale_factor, 0.1);
...
TEST_CASE("test_scale_factor") {
    ...
    const double pot_evap = 5.0; // [mm/h]
	double act_evap_small_scale = calculate_step(water, pot_evap, 0.5, sca);
	double act_evap_large_scale = calculate_step(water, pot_evap, 1.5, sca);
