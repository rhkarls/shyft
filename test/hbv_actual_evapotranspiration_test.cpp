...
TEST_CASE("test_soil_moisture") {
	...
	double act_evap;
	act_evap = calculate_step(0.0, pot_evap, lp, sca);
	TS_ASSERT_DELTA(act_evap, 0.0, shyfttest::EPS);

	act_evap = calculate_step(1.0e8, pot_evap, lp, sca);
...
TEST_CASE("test_snow") {
	...
	const double pot_evap = 5.0; // [mm/h]
	const double lp = 150.0;
	double act_evap_no_snow = calculate_step(soil_moisture, pot_evap, lp, 0.0);
	double act_evap_some_snow = calculate_step(soil_moisture, pot_evap, lp, 0.1);
...
TEST_CASE("test_soil_moisture_threshold") {
	...
	const double pot_evap = 5.0; // [mm/h]
	const double lp = 150.0;
	double act_evap_less_moisture = calculate_step(50, pot_evap, lp, sca);
	double act_evap_more_moisture = calculate_step(100, pot_evap, lp, sca);
